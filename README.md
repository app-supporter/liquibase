...
Spring Data Jpa를 imeimplementation으로 사용할 경우, 캡슐화로 인해 다른 모듈에서 이를 import를 할 수 없기 때문에 불가피하게 api를 사용했습니다. 이는 도메인 모듈과 영속 모듈을 분리할 경우, imeimplementation을 사용할 수 있지만, 이로 인해 늘어나는 코드량, 유지보수에 들어가는 시간을 고려해 분리하지 않았습니다.
